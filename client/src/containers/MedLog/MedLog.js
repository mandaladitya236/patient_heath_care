...
class MedLog extends Component {
  ...
  // Keep track of what user types into bp input field so that input can be grabbed later.
  // If form validation error is showing, remove error from page when user starts typing.
  handleLogbpChange = (event) => {
    this.setState({ 
      logbp: event.target.value,
      logbpError: "",
      formSuccessMessage: "", 
    });
  }
  ...
  handleFormSubmit = event => {
    ...
    // if the bp field is empty when user submits form, show error.
    if (this.state.logbp === "") {
      this.setState({
        logbpError: "Enter a value for bloodpressure (in mmofHg)."
      })
    }
                ...
                <Grid container spacing={16}>
                  <Grid item xs={12} sm={12} md={6}>
                    <LogForm
                      ...
                      handleLogbpChange={this.handleLogbpChange}
                      ...
                      logbpError={this.state.logbpError}
