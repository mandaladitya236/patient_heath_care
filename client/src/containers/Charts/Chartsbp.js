...
class Chartsbp extends React.Component {
  ...
  loadHeightChart(data) {
    const bpChart = new FusionCharts({
      type: 'line',
      renderAt: 'bpContainer',
      className: 'line',
      ...
      dataSource: {
        chart: {
          xaxisname: 'Date',
          yaxisname: "BloodPressure(In mmofHg)",
          yAxisNamePadding: "30",
          chartTopMargin: "40",
          linecolor: "#f8bd19",
          plotgradientcolor: "",
  ...
  loadLogs = () => {
    MedLogAPI.getLogs()
      .then(res => {
        ...
        // const data = this.state.logs.map(log => { return { label: log.date, value: log.bp } });
        const data = this.state.logs.map(log => { return { label: log.date, value: log.heightIn } });
        console.log(data);
        this.loadbpChart(data);
      })
...
export default withStyles(styles)(Chartsbp);
