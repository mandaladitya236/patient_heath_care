...
class Appointments extends Component {
  ...
  handleFormSubmit = event => {
    ...
    if (this.state.appointmentTime === "") {
      ...
      const date=this.state.appointmentDate.split("-");
      const time=this.state.appointmentTime.split(":"); console.log(this.state.appointmentTime);
      const remindTime=new Date(parseInt(date[0]),parseInt(date[1])-1,parseInt(date[2]),parseInt(time[0]),parseInt(time[1]),0);console.log(remindTime);
